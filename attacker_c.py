...
def check_cell_prop(cell, ship_ids, yard_ids, cargo, player_halite):
    """
        ...
        2: means get away with any of other direction
    """
    if cell.ship != None:
...
def randomize(exclude=[]):
    import random
    
    choice = random.choice(list(acts.items()))
...
def agent(obs, config):
    ...
    for ship in ships:
        ...
        if ship_cell.shipyard != None: 
            mine = False
        elif ship_cell_halite < 10:
            mine = False
        ...
        if cargo > 2500 and len(ships) > 2 and ship_cell.shipyard != None and not declared:
            ship.next_action = acts['convert']
            declared = True
            pass
            
        if not declared: ship.next_action = randomize()
    ...
    for yard in shipyards:
        if len(ships) < 3 and player_halite > 2500:
            yard.next_move = acts['spawn']
